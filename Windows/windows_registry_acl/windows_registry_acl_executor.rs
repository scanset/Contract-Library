//! Windows Registry ACL Executor
//!
//! Validates collected registry-key ACL state against criterion
//! expectations. The heavy lifting (RegistryRights mask decoding,
//! denormalisation into `aces`/`allow_identities`/`deny_identities`)
//! happens in the collector; this executor just does ordinary
//! bool/int/string comparisons on the emitted fields.
...
pub struct RegistryAclExecutor {
    contract: CtnContract,
}

impl RegistryAclExecutor {
...
impl CtnExecutor for RegistryAclExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} Registry ACL objects, found {}",
            ...
            if let Some((expected_exists, operation)) = exists_requirement {
                ...
                if !exists_passed {
                    let msg = if expected_exists && !actual_exists {
                        "Registry key not found on this host".to_string()
                    ...
                    failure_messages.push(format!("Registry ACL '{}': {}", object_id, msg));
                    ...
                    state_results.push(StateValidationResult {
                        ...
                        message: format!("Registry ACL '{}': failed (existence)", object_id),
                    });
                    continue;
                }
            }
                    ...
                    let actual_value = match data.get_field(&data_field_name) {
                        ...
                        None => {
                            let msg = if !actual_exists {
                                format!(
                                    "Field '{}' not available (registry key does not exist)",
                                    field.name
                                )
                            } else {
                                format!(
                                    "Field '{}' not collected (source data was null)",
                                    field.name
                                )
                            };
                            all_field_results.push(FieldValidationResult {
                            ...
                            failure_messages
                                .push(format!("Registry ACL '{}': {}", object_id, msg));
                            continue;
                        }
                    };
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Registry ACL validation passed: {} of {} ACLs compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "Registry ACL validation failed:\n  - {}",
