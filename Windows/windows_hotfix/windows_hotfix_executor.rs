//! Windows HotFix Executor
//!
//! Validates collected hotfix state against criterion expectations.
//! The collector already does all date -> days conversion; this
//! executor just does ordinary bool/int/string comparisons on the
//! emitted fields.
...
impl CtnExecutor for HotfixExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} HotFix objects, found {}",
            ...
            if let Some((expected_exists, operation)) = exists_requirement {
                ...
                if !exists_passed {
                    let msg = if expected_exists && !actual_exists {
                        "HotFix not installed on this host".to_string()
                    } else if !expected_exists && actual_exists {
                        "HotFix is installed but should not be".to_string()
                    } else {
                        format!(
                    ...
                    failure_messages.push(format!("HotFix '{}': {}", object_id, msg));
                    ...
                    state_results.push(StateValidationResult {
                        ...
                        message: format!("HotFix '{}': failed (existence)", object_id),
                    });
                    continue;
                }
            }
                    ...
                    let actual_value = match data.get_field(&data_field_name) {
                        ...
                        None => {
                            let msg = if !actual_exists {
                                format!(
                                    "Field '{}' not available (hotfix not installed)",
                                    field.name
                                )
                            } else {
                                format!(
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "HotFix validation passed: {} of {} hotfixes compliant",
                objects_passing,
                state_results.len()
            )
        } else {
