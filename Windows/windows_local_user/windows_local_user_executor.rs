//! Local User Executor (Windows)
//!
//! Validates collected local-account state against criterion expectations.
//! Handles bool policy flags, SID/description string comparisons, and
//! integer "days" comparisons (password_last_set_days, last_logon_days,
//! etc.) using standard arithmetic ops.
...
pub struct LocalUserExecutor {
    contract: CtnContract,
}

impl LocalUserExecutor {
...
impl CtnExecutor for LocalUserExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} user objects, found {}",
        ...
        for (object_id, data) in &collected_data {
            ...
            // If criterion asks for `exists` explicitly, handle it first so we
            // can short-circuit when an account that shouldn't exist is found
            // (or vice versa) without cascading a pile of missing-field noise.
            ...
            if let Some((expected_exists, operation)) = exists_requirement {
                ...
                if !exists_passed {
                    let msg = if expected_exists && !actual_exists {
                        ...
                        "Local user exists but should not".to_string()
                    } else {
                        format!(
                    ...
                    failure_messages.push(format!("User '{}': {}", object_id, msg));
                    ...
                    state_results.push(StateValidationResult {
                        ...
                        message: format!("User '{}': failed (existence)", object_id),
                    });
                    continue;
                }
            }
                    ...
                    let actual_value = match data.get_field(&data_field_name) {
                        ...
                        None => {
                            let msg = if !actual_exists {
                                format!(
                                    "Field '{}' not available (user does not exist)",
                                    field.name
                                )
                            } else {
                                format!(
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Local user validation passed: {} of {} users compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "Local user validation failed:\n  - {}",
