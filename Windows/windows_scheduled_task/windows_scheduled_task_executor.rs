//! Windows Scheduled Task Executor
//!
//! Validates collected scheduled-task state against criterion
//! expectations. The collector already does all date -> days and
//! state-int -> state-string conversion; this executor just does
...
impl CtnExecutor for ScheduledTaskExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} Scheduled Task objects, found {}",
            ...
            if let Some((expected_exists, operation)) = exists_requirement {
                ...
                if !exists_passed {
                    let msg = if expected_exists && !actual_exists {
                        "Scheduled task not found on this host".to_string()
                    } else if !expected_exists && actual_exists {
                        "Scheduled task exists but should not".to_string()
                    } else {
                        format!(
                    ...
                    failure_messages
                        .push(format!("Scheduled Task '{}': {}", object_id, msg));
                    ...
                    state_results.push(StateValidationResult {
                        ...
                        message: format!(
                            "Scheduled Task '{}': failed (existence)",
                            object_id
                        ),
                    });
                    continue;
                    ...
                    let actual_value = match data.get_field(&data_field_name) {
                        ...
                        None => {
                            let msg = if !actual_exists {
                                format!(
                                    "Field '{}' not available (scheduled task does not exist)",
                                    field.name
                                )
                            } else {
                                format!(
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Scheduled Task validation passed: {} of {} tasks compliant",
                objects_passing,
                state_results.len()
            )
        } else {
