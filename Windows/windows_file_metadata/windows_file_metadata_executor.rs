//! Windows File Metadata Executor
//!
//! Validates collected file-metadata state against criterion
//! expectations. Logic is field-agnostic: reads `state_to_data` from the
//! contract and dispatches to generic comparators.
...
impl FileMetadataExecutor {
    ...
    fn compare_values(
        ...
        match (expected, actual) {
            ...
            // ESP backtick literals are always String — coerce to Integer when
            // the collected field is Integer so `size int > \`0\`` works.
            (ResolvedValue::String(exp), ResolvedValue::Integer(act)) => {
                if let Ok(exp_int) = exp.parse::<i64>() {
                    Self::cmp_int(*act, exp_int, operation)
                } else {
                    false
                }
            }
    ...
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} file objects, found {}",
            ...
            if let Some((expected_exists, operation)) = exists_requirement {
                ...
                if !exists_passed {
                    let msg = if expected_exists && !actual_exists {
                        "Path not found on this host".to_string()
                    } else if !expected_exists && actual_exists {
                        "Path exists but should not".to_string()
                    } else {
                        format!(
                    ...
                    failure_messages.push(format!("file '{}': {}", object_id, msg));
                    ...
                    state_results.push(StateValidationResult {
                        ...
                        message: format!("file '{}': failed (existence)", object_id),
                    });
                    continue;
                }
            }
                    ...
                    let actual_value = match data.get_field(&data_field_name) {
                        ...
                        None => {
                            let msg = if !actual_exists {
                                format!(
                                    "Field '{}' not available (path does not exist)",
                                    field.name
                                )
                            } else {
                                format!(
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "File metadata validation passed: {} of {} files compliant",
                objects_passing,
                state_results.len()
            )
        } else {
