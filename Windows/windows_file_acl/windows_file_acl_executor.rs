//! Windows File ACL Executor
//!
//! Validates collected ACL state against criterion expectations. The
//! heavy lifting (rights-mask decoding, denormalisation into
//! `aces`/`allow_identities`/`deny_identities`) happens in the
//! collector; this executor just does ordinary bool/int/string
//! comparisons on the emitted fields.
...
pub struct FileAclExecutor {
    contract: CtnContract,
}

impl FileAclExecutor {
...
impl CtnExecutor for FileAclExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} ACL objects, found {}",
        ...
        for (object_id, data) in &collected_data {
            ...
            if let Some((expected_exists, operation)) = exists_requirement {
                ...
                if !exists_passed {
                    ...
                    failure_messages.push(format!("ACL '{}': {}", object_id, msg));
                    ...
                    state_results.push(StateValidationResult {
                        ...
                        message: format!("ACL '{}': failed (existence)", object_id),
                    });
                    continue;
                }
            }
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "File ACL validation passed: {} of {} ACLs compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "File ACL validation failed:\n  - {}",
