//! Local Group Executor (Windows)
//!
//! Validates collected group state — existence, SID/description,
//! member_count (int), and member-list string predicates (members,
//! member_sids, member_object_classes, member_sources) — against
...
pub struct LocalGroupExecutor {
    contract: CtnContract,
}

impl LocalGroupExecutor {
...
impl CtnExecutor for LocalGroupExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} group objects, found {}",
            ...
            if let Some((expected_exists, operation)) = exists_requirement {
                ...
                if !exists_passed {
                    let msg = if expected_exists && !actual_exists {
                        "Local group not found".to_string()
                    } else if !expected_exists && actual_exists {
                        "Local group exists but should not".to_string()
                    } else {
                        format!(
                    ...
                    state_results.push(StateValidationResult {
                        ...
                        message: format!("Group '{}': failed (existence)", object_id),
                    });
                    continue;
                }
            }
                    ...
                    let actual_value = match data.get_field(&data_field_name) {
                        ...
                        None => {
                            let msg = if !actual_exists {
                                format!(
                                    "Field '{}' not available (group does not exist)",
                                    field.name
                                )
                            } else {
                                format!(
                                    "Field '{}' not collected (unsupported by active \
                                     executor backend)",
                                    field.name
                                )
                            };
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Local group validation passed: {} of {} groups compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "Local group validation failed:\n  - {}",
