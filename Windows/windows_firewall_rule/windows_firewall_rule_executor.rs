//! Windows Firewall Rule Executor
//!
//! Validates collected firewall-rule state against criterion
//! expectations. The collector already translates NetSecurity's
//! integer enums (Enabled, Direction, Action, Profile bitmask,
//! PrimaryStatus) into bool / string values; this executor just does
//! ordinary bool/int/string comparisons on the emitted fields.
...
pub struct FirewallRuleExecutor {
    contract: CtnContract,
}

impl FirewallRuleExecutor {
...
impl CtnExecutor for FirewallRuleExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} Firewall Rule objects, found {}",
            ...
            if let Some((expected_exists, operation)) = exists_requirement {
                ...
                if !exists_passed {
                    let msg = if expected_exists && !actual_exists {
                        "Firewall rule not found on this host".to_string()
                    } else if !expected_exists && actual_exists {
                        "Firewall rule exists but should not".to_string()
                    } else {
                        format!(
                    ...
                    failure_messages
                        .push(format!("Firewall Rule '{}': {}", object_id, msg));
                    ...
                    state_results.push(StateValidationResult {
                        ...
                        message: format!(
                            "Firewall Rule '{}': failed (existence)",
                            object_id
                        ),
                    });
                    continue;
                    ...
                    let actual_value = match data.get_field(&data_field_name) {
                        ...
                        None => {
                            let msg = if !actual_exists {
                                format!(
                                    "Field '{}' not available (firewall rule does not exist)",
                                    field.name
                                )
                            } else {
                                format!(
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Firewall Rule validation passed: {} of {} rules compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "Firewall Rule validation failed:\n  - {}",
