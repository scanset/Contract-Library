//! Windows Firewall Profile Executor
//!
//! Validates collected firewall-profile state against criterion
//! expectations. The collector already translates NetSecurity's
//! GpoBoolean / GpoAction integer enums into bool / string values;
//! this executor just does ordinary bool/int/string comparisons on
//! the emitted fields.
...
pub struct FirewallProfileExecutor {
    contract: CtnContract,
}

impl FirewallProfileExecutor {
...
impl CtnExecutor for FirewallProfileExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} Firewall Profile objects, found {}",
            ...
            if let Some((expected_exists, operation)) = exists_requirement {
                ...
                if !exists_passed {
                    let msg = if expected_exists && !actual_exists {
                        "Firewall profile not found on this host".to_string()
                    } else if !expected_exists && actual_exists {
                        "Firewall profile exists but should not".to_string()
                    } else {
                        format!(
                    ...
                    failure_messages
                        .push(format!("Firewall Profile '{}': {}", object_id, msg));
                    ...
                    state_results.push(StateValidationResult {
                        ...
                        message: format!(
                            "Firewall Profile '{}': failed (existence)",
                            object_id
                        ),
                    });
                    continue;
                    ...
                    let actual_value = match data.get_field(&data_field_name) {
                        ...
                        None => {
                            let msg = if !actual_exists {
                                format!(
                                    "Field '{}' not available (firewall profile does not exist)",
                                    field.name
                                )
                            } else {
                                format!(
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Firewall Profile validation passed: {} of {} profiles compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "Firewall Profile validation failed:\n  - {}",
