//! Executor for the `m365_graph_query` CTN.
//!
...
pub struct M365GraphQueryExecutor {
    contract: CtnContract,
}

impl M365GraphQueryExecutor {
                ...
                for field in &state.fields {
                    ...
                    let actual_value = match data.get_field(&data_field_name) {
                        ...
                        None => {
                            ...
                            failure_messages.push(format!("Graph '{}': {}", object_id, msg));
                            continue;
                        }
                    };
...
impl CtnExecutor for M365GraphQueryExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} Graph queries, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Graph query passed: {} of {} queries returned data",
                objects_passing,
                state_results.len()
            )
        } else {
