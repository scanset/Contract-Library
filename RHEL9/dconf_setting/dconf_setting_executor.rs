...
//  pub use dconf_setting::DconfSettingExecutor;
//
//////////////////////////////////////////////////////
...
pub struct DconfSettingExecutor {
    contract: CtnContract,
}

impl DconfSettingExecutor {
...
impl CtnExecutor for DconfSettingExecutor {
    fn execute_with_contract(
                ...
                for field in &state.fields {
                    ...
                    let actual_value = match data.get_field(&data_field_name) {
                        ...
                        None => {
                            ...
                            failure_messages
                                .push(format!("Setting '{}': {}", object_id, msg));
                            continue;
                        }
                    };
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "dconf setting validation passed: {} of {} settings compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "dconf setting validation failed:\n  - {}",
    ...
    fn validate_collected_data(
        ...
        for data in collected_data.values() {
            if !data.has_field("applicable") {
                return Err(CtnExecutionError::MissingDataField {
                    field: "applicable".to_string(),
                });
            }
        }
        Ok(())
