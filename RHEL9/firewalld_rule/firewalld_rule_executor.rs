...
//  pub use firewalld_rule::FirewalldRuleExecutor;
//
//////////////////////////////////////////////////////
...
pub struct FirewalldRuleExecutor {
    contract: CtnContract,
}

impl FirewalldRuleExecutor {
...
impl CtnExecutor for FirewalldRuleExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} firewalld zones, found {}",
                ...
                for field in &state.fields {
                    ...
                    let actual_value = match data.get_field(&data_field_name) {
                        ...
                        None => {
                            ...
                            failure_messages
                                .push(format!("Firewalld zone '{}': {}", object_id, msg));
                            continue;
                        }
                    };
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Firewalld rule validation passed: {} of {} zones compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "Firewalld rule validation failed:\n  - {}",
