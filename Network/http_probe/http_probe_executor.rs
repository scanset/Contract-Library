...
///
/// pub mod http_probe;
//  pub use http_probe::HttpProbeExecutor;
//
//////////////////////////////////////////////////////
...
impl CtnExecutor for HttpProbeExecutor {
    fn execute_with_contract(
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "HTTP probe validation passed: {} of {} endpoints compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "HTTP probe validation failed:\n  - {}",
