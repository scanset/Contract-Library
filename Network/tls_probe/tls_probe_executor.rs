...
///
/// pub mod tls_probe;
//  pub use tls_probe::TlsProbeExecutor;
//
//////////////////////////////////////////////////////
...
pub struct TlsProbeExecutor {
    contract: CtnContract,
}

impl TlsProbeExecutor {
...
impl CtnExecutor for TlsProbeExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} probes, found {}",
                ...
                for field in &state.fields {
                    ...
                    let actual_value = match data.get_field(&data_field_name) {
                        ...
                        None => {
                            ...
                            failure_messages.push(format!("Probe '{}': {}", object_id, msg));
                            continue;
                        }
                    };
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "TLS probe validation passed: {} of {} endpoints compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "TLS probe validation failed:\n  - {}",
