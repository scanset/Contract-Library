//! Azure Disk Encryption Set Executor
...
pub struct AzDiskEncryptionSetExecutor {
    contract: CtnContract,
}

impl AzDiskEncryptionSetExecutor {
        ...
        for (object_id, data) in collected_data {
            ...
            for state in &criterion.states {
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg =
                            "DES not found, cannot validate record checks".to_string();
                        ...
                        failure_messages.push(format!("DES '{}': {}", object_id, msg));
                        continue;
                    }
...
impl CtnExecutor for AzDiskEncryptionSetExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} disk encryption sets, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "DES validation passed: {} of {} sets compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "DES validation failed:\n  - {}",
