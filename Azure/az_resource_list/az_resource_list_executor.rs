//! Azure ARM Resource List Executor
//!
//! Validates the collected scalar STATE fields (`found`, `resource_count`)
//! against assertions in the policy. The `resources` RecordData carries
//! the projected record array — it's not currently asserted against in
//! the executor, but is preserved on the envelope for inspection and for
//! the engine PR's DISCOVERY-mode iterator.
...
pub struct AzResourceListExecutor {
    contract: CtnContract,
}

impl AzResourceListExecutor {
        ...
        for (object_id, data) in collected_data {
            ...
            for state in &criterion.states {
                for field in &state.fields {
                    // The `resources` RecordData isn't asserted on directly in v1;
                    // it's surfaced on the envelope for downstream consumers.
                    ...
                    let actual_value = match data.get_field(&data_field_name) {
                        ...
                        None => {
                            ...
                            failure_messages
                                .push(format!("Resource list '{}': {}", object_id, msg));
                            continue;
                        }
                    };
...
impl CtnExecutor for AzResourceListExecutor {
    fn execute_with_contract(
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Discovery enumeration passed: {} of {} scopes admitted",
                objects_passing,
                state_results.len()
            )
        } else {
