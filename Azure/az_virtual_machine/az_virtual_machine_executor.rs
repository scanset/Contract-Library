//! Azure Virtual Machine (VM) Executor
...
//  pub use az_virtual_machine::AzVirtualMachineExecutor;
//
///////////////////////////////////////////////////////
...
pub struct AzVirtualMachineExecutor {
    contract: CtnContract,
}

impl AzVirtualMachineExecutor {
        ...
        for (object_id, data) in collected_data {
            ...
            for state in &criterion.states {
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg = "VM not found, cannot validate record checks".to_string();
                        ...
                        failure_messages.push(format!("VM '{}': {}", object_id, msg));
                        continue;
                    }
...
impl CtnExecutor for AzVirtualMachineExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} VMs, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "VM validation passed: {} of {} VMs compliant",
                objects_passing,
                state_results.len()
            )
        } else {
