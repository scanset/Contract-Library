//! Azure Activity Log Alert Executor
...
pub struct AzActivityLogAlertExecutor {
    contract: CtnContract,
}

impl AzActivityLogAlertExecutor {
        ...
        for (object_id, data) in collected_data {
            ...
            for state in &criterion.states {
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg =
                            "Alert not found, cannot validate record checks".to_string();
                        ...
                        failure_messages.push(format!("Alert '{}': {}", object_id, msg));
                        continue;
                    }
...
impl CtnExecutor for AzActivityLogAlertExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} alerts, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Activity log alert validation passed: {} of {} alerts compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "Activity log alert validation failed:\n  - {}",
