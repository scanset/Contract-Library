...
pub struct AzLoadBalancerExecutor {
    contract: CtnContract,
}

impl AzLoadBalancerExecutor {
        ...
        for (object_id, data) in collected_data {
            ...
            for state in &criterion.states {
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg = "LB not found, cannot validate record checks".to_string();
                        ...
                        failure_messages.push(format!("LB '{}': {}", object_id, msg));
                        continue;
                    }
...
impl CtnExecutor for AzLoadBalancerExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} load balancers, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "LB validation passed: {} of {} load balancers compliant",
                objects_passing,
                state_results.len()
            )
        } else {
