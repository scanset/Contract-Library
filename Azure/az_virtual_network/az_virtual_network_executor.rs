//! Azure Virtual Network (VNet) Executor
...
//  pub use az_virtual_network::AzVirtualNetworkExecutor;
//
///////////////////////////////////////////////////////
...
pub struct AzVirtualNetworkExecutor {
    contract: CtnContract,
}

impl AzVirtualNetworkExecutor {
        ...
        for (object_id, data) in collected_data {
            ...
            for state in &criterion.states {
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg = "VNet not found, cannot validate record checks".to_string();
                        ...
                        failure_messages.push(format!("VNet '{}': {}", object_id, msg));
                        continue;
                    }
...
impl CtnExecutor for AzVirtualNetworkExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} VNets, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "VNet validation passed: {} of {} VNets compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "VNet validation failed:\n  - {}",
