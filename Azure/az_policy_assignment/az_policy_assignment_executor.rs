//! Azure Policy Assignment Executor
...
//  pub use az_policy_assignment::AzPolicyAssignmentExecutor;
//
///////////////////////////////////////////////////////
...
pub struct AzPolicyAssignmentExecutor {
    contract: CtnContract,
}

impl AzPolicyAssignmentExecutor {
        ...
        for (object_id, data) in collected_data {
            ...
            for state in &criterion.states {
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg =
                            "Policy assignment not found, cannot validate record checks".to_string();
...
impl CtnExecutor for AzPolicyAssignmentExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} policy assignments, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Policy assignment validation passed: {} of {} assignments compliant",
                objects_passing,
                state_results.len()
            )
        } else {
