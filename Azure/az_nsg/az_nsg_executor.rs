//! Azure Network Security Group (NSG) Executor
        ...
        for (object_id, data) in collected_data {
            ...
            for state in &criterion.states {
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg = "NSG not found, cannot validate record checks".to_string();
                        ...
                        failure_messages.push(format!("NSG '{}': {}", object_id, msg));
                        continue;
                    }
...
impl CtnExecutor for AzNsgExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} NSGs, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "NSG validation passed: {} of {} NSGs compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "NSG validation failed:\n  - {}",
