//! Azure Log Analytics Workspace (LAW) Executor
...
//  pub use az_log_analytics_workspace::AzLogAnalyticsWorkspaceExecutor;
//
///////////////////////////////////////////////////////
...
pub struct AzLogAnalyticsWorkspaceExecutor {
    contract: CtnContract,
}

impl AzLogAnalyticsWorkspaceExecutor {
        ...
        for (object_id, data) in collected_data {
            ...
            for state in &criterion.states {
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg =
                            "Workspace not found, cannot validate record checks".to_string();
                        ...
                        failure_messages.push(format!("LAW '{}': {}", object_id, msg));
                        continue;
                    }
...
impl CtnExecutor for AzLogAnalyticsWorkspaceExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} workspaces, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "LAW validation passed: {} of {} workspaces compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "LAW validation failed:\n  - {}",
