...
pub struct AzBastionHostExecutor {
    contract: CtnContract,
}

impl AzBastionHostExecutor {
        ...
        for (object_id, data) in collected_data {
            ...
            for state in &criterion.states {
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg = "Bastion not found, cannot validate record checks".to_string();
                        ...
                        failure_messages.push(format!("Bastion '{}': {}", object_id, msg));
                        continue;
                    }
...
impl CtnExecutor for AzBastionHostExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} bastion hosts, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Bastion validation passed: {} of {} hosts compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "Bastion validation failed:\n  - {}",
