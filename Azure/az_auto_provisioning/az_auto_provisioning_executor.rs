//! Azure Security Auto-Provisioning Setting Executor
...
pub struct AzAutoProvisioningExecutor {
    contract: CtnContract,
}

impl AzAutoProvisioningExecutor {
        ...
        for (object_id, data) in collected_data {
            ...
            for state in &criterion.states {
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg = "Auto-provisioning setting not found, cannot validate record checks"
                            .to_string();
                        ...
                        failure_messages.push(format!("AutoProv '{}': {}", object_id, msg));
                        continue;
                    }
...
impl CtnExecutor for AzAutoProvisioningExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} settings, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Auto-provisioning validation passed: {} of {} settings compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "Auto-provisioning validation failed:\n  - {}",
