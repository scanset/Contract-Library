...
pub struct AzManagedDiskExecutor {
    contract: CtnContract,
}

impl AzManagedDiskExecutor {
        ...
        for (object_id, data) in collected_data {
            ...
            for state in &criterion.states {
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg =
                            "Disk not found, cannot validate record checks".to_string();
                        ...
                        failure_messages.push(format!("Disk '{}': {}", object_id, msg));
                        continue;
                    }
...
impl CtnExecutor for AzManagedDiskExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} disks, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Disk validation passed: {} of {} disks compliant",
                objects_passing,
                state_results.len()
            )
        } else {
