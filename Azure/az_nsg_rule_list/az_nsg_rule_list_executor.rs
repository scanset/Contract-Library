//! Azure NSG Rule List Executor — same shape as the other list executors.
...
pub struct AzNsgRuleListExecutor {
    contract: CtnContract,
}

impl AzNsgRuleListExecutor {
                ...
                for field in &state.fields {
                    ...
                    let actual_value = match data.get_field(&data_field_name) {
                        ...
                        None => {
                            ...
                            failure_messages
                                .push(format!("NSG rule list '{}': {}", object_id, msg));
                            continue;
                        }
                    };
...
impl CtnExecutor for AzNsgRuleListExecutor {
    fn execute_with_contract(
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "NSG rule enumeration passed: {} of {} scopes admitted",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "NSG rule enumeration failed:\n  - {}",
