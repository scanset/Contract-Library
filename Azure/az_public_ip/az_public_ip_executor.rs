//! Azure Public IP Address Executor
...
pub struct AzPublicIpExecutor {
    contract: CtnContract,
}

impl AzPublicIpExecutor {
        ...
        for (object_id, data) in collected_data {
            ...
            for state in &criterion.states {
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg = "Public IP not found, cannot validate record checks".to_string();
                        ...
                        failure_messages.push(format!("PIP '{}': {}", object_id, msg));
                        continue;
                    }
...
impl CtnExecutor for AzPublicIpExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} public IPs, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Public IP validation passed: {} of {} IPs compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "Public IP validation failed:\n  - {}",
