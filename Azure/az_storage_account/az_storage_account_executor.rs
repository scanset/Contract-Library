//! Azure Storage Account Executor
...
//  pub use az_storage_account::AzStorageAccountExecutor;
//
///////////////////////////////////////////////////////
...
pub struct AzStorageAccountExecutor {
    contract: CtnContract,
}

impl AzStorageAccountExecutor {
        ...
        for (object_id, data) in collected_data {
            ...
            for state in &criterion.states {
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg =
                            "Storage account not found, cannot validate record checks".to_string();
                        ...
                        failure_messages
                            .push(format!("Storage account '{}': {}", object_id, msg));
                        continue;
                    }
...
impl CtnExecutor for AzStorageAccountExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} storage accounts, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Storage account validation passed: {} of {} accounts compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "Storage account validation failed:\n  - {}",
