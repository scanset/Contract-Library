//! Azure Application Gateway Executor
...
//  pub use az_application_gateway::AzApplicationGatewayExecutor;
//
///////////////////////////////////////////////////////
...
pub struct AzApplicationGatewayExecutor {
    contract: CtnContract,
}

impl AzApplicationGatewayExecutor {
        ...
        for (object_id, data) in collected_data {
            ...
            for state in &criterion.states {
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg = "AppGW not found, cannot validate record checks".to_string();
                        ...
                        failure_messages.push(format!("AppGW '{}': {}", object_id, msg));
                        continue;
                    }
...
impl CtnExecutor for AzApplicationGatewayExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} gateways, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "AppGW validation passed: {} of {} gateways compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "AppGW validation failed:\n  - {}",
