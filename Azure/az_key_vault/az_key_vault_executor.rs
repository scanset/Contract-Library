...
//  pub use az_key_vault::AzKeyVaultExecutor;
//
///////////////////////////////////////////////////////
...
pub struct AzKeyVaultExecutor {
    contract: CtnContract,
}

impl AzKeyVaultExecutor {
        ...
        for (object_id, data) in collected_data {
            ...
            for state in &criterion.states {
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg = "Key Vault not found, cannot validate record checks".to_string();
                        ...
                        failure_messages.push(format!("Key Vault '{}': {}", object_id, msg));
                        continue;
                    }
...
impl CtnExecutor for AzKeyVaultExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} key vaults, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Key Vault validation passed: {} of {} vaults compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "Key Vault validation failed:\n  - {}",
