//! Azure Policy Assignment List Executor
//!
//! Validates `found` + `assignment_count`. The `assignments` RecordData
//! is preserved on the envelope but not asserted on directly. Same shape
//! as the other list executors.
...
pub struct AzPolicyAssignmentListExecutor {
    contract: CtnContract,
}

impl AzPolicyAssignmentListExecutor {
        ...
        for (object_id, data) in collected_data {
            ...
            for state in &criterion.states {
                for field in &state.fields {
                    ...
                    if !passed {
                        failure_messages
                            .push(format!("Policy assignment list '{}': {}", object_id, msg));
                    }
                    all_field_results.push(FieldValidationResult {
                        field_name: field.name.clone(),
                        expected_value: field.value.clone(),
...
impl CtnExecutor for AzPolicyAssignmentListExecutor {
    fn execute_with_contract(
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Policy assignment enumeration passed: {} of {} scopes admitted",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "Policy assignment enumeration failed:\n  - {}",
