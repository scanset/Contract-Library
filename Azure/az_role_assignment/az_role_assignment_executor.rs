...
pub struct AzRoleAssignmentExecutor {
    contract: CtnContract,
}

impl AzRoleAssignmentExecutor {
...
impl CtnExecutor for AzRoleAssignmentExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} Role assignments, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        let (state_results, failure_messages) =
            self.run_checks(criterion, &collected_data, "Role assignment");
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Role assignment validation passed: {} of {} compliant",
                objects_passing,
                state_results.len()
            )
        } else {
