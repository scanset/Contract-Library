//! Azure Security Contact Executor
...
pub struct AzSecurityContactExecutor {
    contract: CtnContract,
}

impl AzSecurityContactExecutor {
        ...
        for (object_id, data) in collected_data {
            ...
            for state in &criterion.states {
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg =
                            "Security contact not found, cannot validate record checks".to_string();
                        ...
                        failure_messages.push(format!("SecurityContact '{}': {}", object_id, msg));
                        continue;
                    }
...
impl CtnExecutor for AzSecurityContactExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} contacts, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Security contact validation passed: {} of {} contacts compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "Security contact validation failed:\n  - {}",
