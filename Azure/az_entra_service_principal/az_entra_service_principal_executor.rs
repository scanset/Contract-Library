...
pub struct AzEntraServicePrincipalExecutor {
    contract: CtnContract,
}

impl AzEntraServicePrincipalExecutor {
...
impl CtnExecutor for AzEntraServicePrincipalExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} Entra service principals, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        let (state_results, failure_messages) =
            self.run_checks(criterion, &collected_data, "Entra service principal");
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Entra service principal validation passed: {} of {} compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "Entra service principal validation failed:\n  - {}",
