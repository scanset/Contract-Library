...
pub struct AzResourceGroupExecutor {
    contract: CtnContract,
}

impl AzResourceGroupExecutor {
        ...
        for (object_id, data) in collected_data {
            ...
            for state in &criterion.states {
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg = "Resource group not found, cannot validate record checks"
                            .to_string();
                        ...
                        failure_messages.push(format!("Resource group '{}': {}", object_id, msg));
                        continue;
                    }
                    match data.get_field("resource") {
                        ...
                        _ => {
                            ...
                            failure_messages
                                .push(format!("Resource group '{}': {}", object_id, msg));
                        }
                    }
                }
...
impl CtnExecutor for AzResourceGroupExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} resource groups, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Resource group validation passed: {} of {} groups compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "Resource group validation failed:\n  - {}",
