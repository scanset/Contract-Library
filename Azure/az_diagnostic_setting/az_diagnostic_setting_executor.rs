//! Azure Diagnostic Setting Executor
...
//  pub use az_diagnostic_setting::AzDiagnosticSettingExecutor;
//
///////////////////////////////////////////////////////
...
pub struct AzDiagnosticSettingExecutor {
    contract: CtnContract,
}

impl AzDiagnosticSettingExecutor {
        ...
        for (object_id, data) in collected_data {
            ...
            for state in &criterion.states {
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg =
                            "Diagnostic setting not found, cannot validate record checks".to_string();
...
impl CtnExecutor for AzDiagnosticSettingExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} diagnostic settings, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Diagnostic setting validation passed: {} of {} settings compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "Diagnostic setting validation failed:\n  - {}",
