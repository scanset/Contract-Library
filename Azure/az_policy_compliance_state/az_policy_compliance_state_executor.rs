//! Azure Policy Compliance State Executor

///////////////////////////////////////////////////////
//
//  mod.rs additions (agent/src/contract_kit/executors/mod.rs)
//
//  pub mod az_policy_compliance_state;
//  pub use az_policy_compliance_state::AzPolicyComplianceStateExecutor;
//
///////////////////////////////////////////////////////

//! Standard rich executor: combines per-field scalar checks with
//! optional `record_checks` against the full summarize response. Mirrors
//! the pattern used by `AzPolicyAssignmentExecutor`. The only domain
//! twist here is the semantics of `found`: it is derived from the
//! response (`policyAssignments.len() > 0`) rather than from a stderr
//! NotFound pattern, because Policy Insights returns 200/empty for
//! missing scopes.
...
pub struct AzPolicyComplianceStateExecutor {
    contract: CtnContract,
}

impl AzPolicyComplianceStateExecutor {
        ...
        for (object_id, data) in collected_data {
            ...
            for state in &criterion.states {
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg =
                            "Policy compliance state has no assignments at scope, \
                             cannot validate record checks"
                                .to_string();
...
impl CtnExecutor for AzPolicyComplianceStateExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} scopes, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Policy compliance state validation passed: {} of {} scopes compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "Policy compliance state validation failed:\n  - {}",
