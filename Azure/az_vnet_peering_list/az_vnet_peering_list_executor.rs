//! Azure VNet Peering List Executor — same shape as the other list executors.
...
pub struct AzVnetPeeringListExecutor {
    contract: CtnContract,
}

impl AzVnetPeeringListExecutor {
                ...
                for field in &state.fields {
                    ...
                    let actual_value = match data.get_field(&data_field_name) {
                        ...
                        None => {
                            ...
                            failure_messages
                                .push(format!("Peering list '{}': {}", object_id, msg));
                            continue;
                        }
                    };
...
impl CtnExecutor for AzVnetPeeringListExecutor {
    fn execute_with_contract(
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Peering enumeration passed: {} of {} scopes admitted",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "Peering enumeration failed:\n  - {}",
