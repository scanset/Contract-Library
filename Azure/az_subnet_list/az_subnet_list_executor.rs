//! Azure Subnet List Executor — same shape as the other list executors.
...
pub struct AzSubnetListExecutor {
    contract: CtnContract,
}

impl AzSubnetListExecutor {
                ...
                for field in &state.fields {
                    ...
                    let actual_value = match data.get_field(&data_field_name) {
                        ...
                        None => {
                            ...
                            failure_messages
                                .push(format!("Subnet list '{}': {}", object_id, msg));
                            continue;
                        }
                    };
...
impl CtnExecutor for AzSubnetListExecutor {
    fn execute_with_contract(
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Subnet enumeration passed: {} of {} scopes admitted",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "Subnet enumeration failed:\n  - {}",
