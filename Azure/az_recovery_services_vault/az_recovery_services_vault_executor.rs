//! Azure Recovery Services Vault Executor
...
pub struct AzRecoveryServicesVaultExecutor {
    contract: CtnContract,
}

impl AzRecoveryServicesVaultExecutor {
        ...
        for (object_id, data) in collected_data {
            ...
            for state in &criterion.states {
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg =
                            "RSV not found, cannot validate record checks".to_string();
                        ...
                        failure_messages.push(format!("RSV '{}': {}", object_id, msg));
                        continue;
                    }
...
impl CtnExecutor for AzRecoveryServicesVaultExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} vaults, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "RSV validation passed: {} of {} vaults compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "RSV validation failed:\n  - {}",
