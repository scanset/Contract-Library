...
pub struct AzEntraGroupExecutor {
    contract: CtnContract,
}

impl AzEntraGroupExecutor {
...
impl CtnExecutor for AzEntraGroupExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} Entra groups, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        let (state_results, failure_messages) =
            self.run_checks(criterion, &collected_data, "Entra group");
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Entra group validation passed: {} of {} compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "Entra group validation failed:\n  - {}",
