//! Azure Role Assignment List Executor
//!
//! Validates the collected scalar STATE fields (`found`, `role_count`)
//! against assertions in the policy. The `roles` RecordData passes
//! through to the envelope but isn't asserted on directly. Same shape as
//! `az_resource_list` / `az_resource_group_list` executors.
...
pub struct AzRoleAssignmentListExecutor {
    contract: CtnContract,
}

impl AzRoleAssignmentListExecutor {
        ...
        for (object_id, data) in collected_data {
            ...
            for state in &criterion.states {
                for field in &state.fields {
                    ...
                    if !passed {
                        failure_messages
                            .push(format!("Role assignment list '{}': {}", object_id, msg));
                    }
                    all_field_results.push(FieldValidationResult {
                        field_name: field.name.clone(),
                        expected_value: field.value.clone(),
...
impl CtnExecutor for AzRoleAssignmentListExecutor {
    fn execute_with_contract(
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Role assignment enumeration passed: {} of {} scopes admitted",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "Role assignment enumeration failed:\n  - {}",
