//! Azure Diagnostic Setting List Executor — same shape as the other list executors.
...
pub struct AzDiagnosticSettingListExecutor {
    contract: CtnContract,
}

impl AzDiagnosticSettingListExecutor {
        ...
        for (object_id, data) in collected_data {
            ...
            for state in &criterion.states {
                for field in &state.fields {
                    ...
                    if !passed {
                        failure_messages
                            .push(format!("Diagnostic settings list '{}': {}", object_id, msg));
                    }
                    all_field_results.push(FieldValidationResult {
                        field_name: field.name.clone(),
                        expected_value: field.value.clone(),
...
impl CtnExecutor for AzDiagnosticSettingListExecutor {
    fn execute_with_contract(
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Diagnostic settings enumeration passed: {} of {} scopes admitted",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "Diagnostic settings enumeration failed:\n  - {}",
