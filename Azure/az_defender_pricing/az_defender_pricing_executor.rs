//! Azure Defender for Cloud Pricing Executor
...
pub struct AzDefenderPricingExecutor {
    contract: CtnContract,
}

impl AzDefenderPricingExecutor {
        ...
        for (object_id, data) in collected_data {
            ...
            for state in &criterion.states {
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg =
                            "Defender plan not found, cannot validate record checks".to_string();
                        ...
                        failure_messages.push(format!("DefenderPlan '{}': {}", object_id, msg));
                        continue;
                    }
...
impl CtnExecutor for AzDefenderPricingExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} plans, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Defender pricing validation passed: {} of {} plans compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "Defender pricing validation failed:\n  - {}",
