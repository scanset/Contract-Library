...
//  pub use openssl_cert::OpensslCertExecutor;
//
//////////////////////////////////////////////////////
...
pub struct OpensslCertExecutor {
    contract: CtnContract,
}

impl OpensslCertExecutor {
...
impl CtnExecutor for OpensslCertExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} certificates, found {}",
                ...
                for field in &state.fields {
                    ...
                    let actual_value = match data.get_field(&data_field_name) {
                        ...
                        None => {
                            ...
                            failure_messages
                                .push(format!("Certificate '{}': {}", object_id, msg));
                            continue;
                        }
                    };
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "OpenSSL certificate validation passed: {} of {} certificates compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "OpenSSL certificate validation failed:\n  - {}",
