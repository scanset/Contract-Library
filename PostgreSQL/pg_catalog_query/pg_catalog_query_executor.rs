//! PostgreSQL Catalog Query Executor
...
//  pub use pg_catalog_query::PgCatalogQueryExecutor;
//
//////////////////////////////////////////////////////
...
pub struct PgCatalogQueryExecutor {
    contract: CtnContract,
}

impl PgCatalogQueryExecutor {
...
impl CtnExecutor for PgCatalogQueryExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} queries, found {}",
                ...
                for field in &state.fields {
                    ...
                    let actual_value = match data.get_field(&data_field_name) {
                        ...
                        None => {
                            ...
                            failure_messages.push(format!("Query '{}': {}", object_id, msg));
                            continue;
                        }
                    };
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "PostgreSQL catalog query validation passed: {} of {} queries compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "PostgreSQL catalog query validation failed:\n  - {}",
