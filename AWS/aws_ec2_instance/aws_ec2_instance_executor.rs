...
//  pub use aws_ec2_instance::AwsEc2InstanceExecutor;
//
//////////////////////////////////////////////////////
...
pub struct AwsEc2InstanceExecutor {
    contract: CtnContract,
}

impl AwsEc2InstanceExecutor {
...
impl CtnExecutor for AwsEc2InstanceExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} backup instances, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        let (state_results, failure_messages) =
            self.run_checks(criterion, &collected_data, "Instance");
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Backup vault validation passed: {} of {} instances compliant",
                objects_passing,
                state_results.len()
            )
        } else {
