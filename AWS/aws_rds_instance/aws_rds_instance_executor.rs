//! AWS RDS Instance Executor
//!
//! Validates RDS instance configurations using both scalar field checks
...
//  pub use aws_rds_instance::AwsRdsInstanceExecutor;
//
//////////////////////////////////////////////////////
...
pub struct AwsRdsInstanceExecutor {
    contract: CtnContract,
}

impl AwsRdsInstanceExecutor {
...
impl CtnExecutor for AwsRdsInstanceExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} RDS instances, found {}",
        ...
        for (object_id, data) in &collected_data {
            ...
            for state in &criterion.states {
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg =
                            "RDS instance not found, cannot validate record checks".to_string();
                        ...
                        failure_messages.push(format!("RDS '{}': {}", object_id, msg));
                        continue;
                    }
                    ...
                    for result in &validation_results {
                        ...
                        if !result.passed {
                            failure_messages
                                .push(format!("RDS '{}': {}", object_id, result.message));
                        }
                    }
                }
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "RDS instance validation passed: {} of {} instances compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "RDS instance validation failed:\n  - {}",
