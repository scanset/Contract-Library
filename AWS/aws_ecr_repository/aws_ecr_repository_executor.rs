...
//  pub use aws_ecr_repository::AwsEcrRepositoryExecutor;
//
//////////////////////////////////////////////////////
...
pub struct AwsEcrRepositoryExecutor {
    contract: CtnContract,
}

impl AwsEcrRepositoryExecutor {
...
impl CtnExecutor for AwsEcrRepositoryExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} ECR repos, found {}",
        ...
        for (object_id, data) in &collected_data {
            ...
            for state in &criterion.states {
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg = "ECR repo not found, cannot validate record checks".to_string();
                        ...
                        failure_messages.push(format!("ECR '{}': {}", object_id, msg));
                        continue;
                    }
                    ...
                    for result in &validation_results {
                        ...
                        if !result.passed {
                            failure_messages
                                .push(format!("ECR '{}': {}", object_id, result.message));
                        }
                    }
                }
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "ECR repository validation passed: {} of {} repos compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "ECR repository validation failed:\n  - {}",
