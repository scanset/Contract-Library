//! AWS Inspector2 Account Executor
//!
//! Validates Inspector2 scan configuration and coverage using both scalar
//! field checks and record check validation on the merged response.
...
//  pub use aws_inspector2_account::AwsInspector2AccountExecutor;
//
//////////////////////////////////////////////////////
...
/// Executor for aws_inspector2_account validation
pub struct AwsInspector2AccountExecutor {
    contract: CtnContract,
}

impl AwsInspector2AccountExecutor {
...
impl CtnExecutor for AwsInspector2AccountExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} Inspector2 accounts, found {}",
        ...
        for (object_id, data) in &collected_data {
            ...
            for state in &criterion.states {
                ...
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg =
                            "Inspector2 not enabled, cannot validate record checks".to_string();
                        ...
                        failure_messages.push(format!("Inspector2 '{}': {}", object_id, msg));
                        continue;
                    }
                    ...
                    for result in &validation_results {
                        ...
                        if !result.passed {
                            failure_messages
                                .push(format!("Inspector2 '{}': {}", object_id, result.message));
                        }
                    }
                }
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Inspector2 validation passed: {} of {} accounts compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "Inspector2 validation failed:\n  - {}",
