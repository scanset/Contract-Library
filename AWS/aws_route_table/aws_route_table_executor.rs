//! AWS Route Table Executor
//!
//! Validates route table configurations using both scalar field checks
...
//  pub use aws_route_table::AwsRouteTableExecutor;
//
//////////////////////////////////////////////////////
...
/// Executor for aws_route_table validation
pub struct AwsRouteTableExecutor {
    contract: CtnContract,
}

impl AwsRouteTableExecutor {
    pub fn new(contract: CtnContract) -> Self {
        Self { contract }
    }

    /// Compare values for scalar fields
    fn compare_values(
        &self,
...
impl CtnExecutor for AwsRouteTableExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} route tables, found {}",
        ...
        for (object_id, data) in &collected_data {
            ...
            for state in &criterion.states {
                ...
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg =
                            "Route table not found, cannot validate record checks".to_string();
                        ...
                        failure_messages.push(format!("RT '{}': {}", object_id, msg));
                        continue;
                    }
                    ...
                    for result in &validation_results {
                        ...
                        if !result.passed {
                            failure_messages
                                .push(format!("RT '{}': {}", object_id, result.message));
                        }
                    }
                }

                // Handle scalar field checks
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Route table validation passed: {} of {} tables compliant",
                objects_passing,
                state_results.len()
            )
        } else {
