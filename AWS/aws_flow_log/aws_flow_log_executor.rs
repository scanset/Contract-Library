//! AWS VPC Flow Log Executor
//!
//! Validates VPC Flow Log configurations using both scalar field checks
...
//  pub use aws_flow_log::AwsFlowLogExecutor;
//
//////////////////////////////////////////////////////
...
/// Executor for aws_flow_log validation
pub struct AwsFlowLogExecutor {
    contract: CtnContract,
}

impl AwsFlowLogExecutor {
...
impl CtnExecutor for AwsFlowLogExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} flow logs, found {}",
        ...
        for (object_id, data) in &collected_data {
            ...
            for state in &criterion.states {
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg = "Flow log not found, cannot validate record checks".to_string();
                        ...
                        failure_messages.push(format!("FL '{}': {}", object_id, msg));
                        continue;
                    }
                    ...
                    for result in &validation_results {
                        ...
                        if !result.passed {
                            failure_messages
                                .push(format!("FL '{}': {}", object_id, result.message));
                        }
                    }
                }
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Flow log validation passed: {} of {} flow logs compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "Flow log validation failed:\n  - {}",
