...
//  pub use aws_network_acl::AwsNetworkAclExecutor;
//
//////////////////////////////////////////////////////
...
pub struct AwsNetworkAclExecutor {
    contract: CtnContract,
}

impl AwsNetworkAclExecutor {
...
impl CtnExecutor for AwsNetworkAclExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} backup acls, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        let (state_results, failure_messages) = self.run_checks(criterion, &collected_data, "Acl");
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Backup vault validation passed: {} of {} acls compliant",
                objects_passing,
                state_results.len()
            )
        } else {
