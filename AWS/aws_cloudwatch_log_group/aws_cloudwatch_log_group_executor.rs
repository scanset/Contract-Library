//! AWS CloudWatch Log Group Executor
...
//  pub use aws_cloudwatch_log_group::AwsCloudwatchLogGroupExecutor;
//
//////////////////////////////////////////////////////
...
pub struct AwsCloudwatchLogGroupExecutor {
    contract: CtnContract,
}

impl AwsCloudwatchLogGroupExecutor {
...
impl CtnExecutor for AwsCloudwatchLogGroupExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} log groups, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        for (object_id, data) in &collected_data {
            ...
            for state in &criterion.states {
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg = "Log group not found, cannot validate record checks".to_string();
                        ...
                        failure_messages.push(format!("LogGroup '{}': {}", object_id, msg));
                        continue;
                    }
                    ...
                    for result in &results {
                        ...
                        if !result.passed {
                            failure_messages
                                .push(format!("LogGroup '{}': {}", object_id, result.message));
                        }
                    }
                }
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "CloudWatch log group validation passed: {} of {} groups compliant",
                objects_passing,
                state_results.len()
            )
        } else {
