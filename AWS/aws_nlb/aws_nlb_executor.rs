//! AWS Network Load Balancer (NLB) Executor
//!
//! Validates NLB configurations using both scalar field checks
...
//! ```rust
//! pub mod aws_nlb;
//! pub use aws_nlb::AwsNlbExecutor;
//! ```
...
/// Executor for aws_nlb validation
pub struct AwsNlbExecutor {
...
impl CtnExecutor for AwsNlbExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} NLBs, found {}",
        ...
        for (object_id, data) in &collected_data {
            ...
            for state in &criterion.states {
                ...
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg = "NLB not found, cannot validate record checks".to_string();
                        ...
                        failure_messages.push(format!("NLB '{}': {}", object_id, msg));
                        continue;
                    }
                    ...
                    for result in &validation_results {
                        ...
                        if !result.passed {
                            failure_messages
                                .push(format!("NLB '{}': {}", object_id, result.message));
                        }
                    }
                }
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "NLB validation passed: {} of {} NLBs compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "NLB validation failed:\n  - {}",
