...
//  pub use aws_eks_cluster::AwsEksClusterExecutor;
//
//////////////////////////////////////////////////////
...
pub struct AwsEksClusterExecutor {
    contract: CtnContract,
}

impl AwsEksClusterExecutor {
...
impl CtnExecutor for AwsEksClusterExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} EKS clusters, found {}",
        ...
        for (object_id, data) in &collected_data {
            ...
            for state in &criterion.states {
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg =
                            "EKS cluster not found, cannot validate record checks".to_string();
                        ...
                        failure_messages.push(format!("EKS '{}': {}", object_id, msg));
                        continue;
                    }
                    ...
                    for result in &validation_results {
                        ...
                        if !result.passed {
                            failure_messages
                                .push(format!("EKS '{}': {}", object_id, result.message));
                        }
                    }
                }
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "EKS cluster validation passed: {} of {} clusters compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "EKS cluster validation failed:\n  - {}",
