//! AWS CloudTrail Executor
//!
//! Validates CloudTrail trail configurations using both scalar field checks
//! and record check validation on the merged config + status response.
...
//  pub use aws_cloudtrail::AwsCloudtrailExecutor;
//
//////////////////////////////////////////////////////
...
/// Executor for aws_cloudtrail validation
pub struct AwsCloudtrailExecutor {
    contract: CtnContract,
}

impl AwsCloudtrailExecutor {
...
impl CtnExecutor for AwsCloudtrailExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} trails, found {}",
        ...
        for (object_id, data) in &collected_data {
            ...
            for state in &criterion.states {
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg = "Trail not found, cannot validate record checks".to_string();
                        ...
                        failure_messages.push(format!("Trail '{}': {}", object_id, msg));
                        continue;
                    }
                    ...
                    for result in &validation_results {
                        ...
                        if !result.passed {
                            failure_messages
                                .push(format!("Trail '{}': {}", object_id, result.message));
                        }
                    }
                }
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "CloudTrail validation passed: {} of {} trails compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "CloudTrail validation failed:\n  - {}",
