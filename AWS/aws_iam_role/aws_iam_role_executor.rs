//! AWS IAM Role Executor
//!
//! Validates IAM Role configurations using both scalar field checks
//! and record check validation on the merged role + policies response.
...
//  pub use aws_iam_role::AwsIamRoleExecutor;
//
//////////////////////////////////////////////////////
...
/// Executor for aws_iam_role validation
pub struct AwsIamRoleExecutor {
    contract: CtnContract,
}

impl AwsIamRoleExecutor {
...
impl CtnExecutor for AwsIamRoleExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} IAM roles, found {}",
        ...
        for (object_id, data) in &collected_data {
            ...
            for state in &criterion.states {
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg = "IAM role not found, cannot validate record checks".to_string();
                        ...
                        failure_messages.push(format!("Role '{}': {}", object_id, msg));
                        continue;
                    }
                    ...
                    for result in &validation_results {
                        ...
                        if !result.passed {
                            failure_messages
                                .push(format!("Role '{}': {}", object_id, result.message));
                        }
                    }
                }
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "IAM role validation passed: {} of {} roles compliant",
                objects_passing,
                state_results.len()
            )
        } else {
