//! AWS CloudWatch EventBridge Rule Executor
...
//  pub use aws_cloudwatch_event_rule::AwsCloudwatchEventRuleExecutor;
//
//////////////////////////////////////////////////////
...
pub struct AwsCloudwatchEventRuleExecutor {
    contract: CtnContract,
}

impl AwsCloudwatchEventRuleExecutor {
...
impl CtnExecutor for AwsCloudwatchEventRuleExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} event rules, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        for (object_id, data) in &collected_data {
            ...
            for state in &criterion.states {
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg = "Event rule not found, cannot validate record checks".to_string();
                        ...
                        failure_messages.push(format!("Rule '{}': {}", object_id, msg));
                        continue;
                    }
                    ...
                    for result in &results {
                        ...
                        if !result.passed {
                            failure_messages
                                .push(format!("Rule '{}': {}", object_id, result.message));
                        }
                    }
                }
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "EventBridge rule validation passed: {} of {} rules compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "EventBridge rule validation failed:\n  - {}",
