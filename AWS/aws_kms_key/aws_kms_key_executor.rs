...
///
/// pub mod aws_kms_key;
//  pub use aws_kms_key::AwsKmsKeyExecutor;
//
//////////////////////////////////////////////////////
...
pub struct AwsKmsKeyExecutor {
    contract: CtnContract,
}

impl AwsKmsKeyExecutor {
...
impl CtnExecutor for AwsKmsKeyExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} backup keys, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        let (state_results, failure_messages) = self.run_checks(criterion, &collected_data, "Key");
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Backup vault validation passed: {} of {} keys compliant",
                objects_passing,
                state_results.len()
            )
        } else {
