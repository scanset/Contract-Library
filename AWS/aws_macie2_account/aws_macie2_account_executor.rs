//! AWS Macie2 Account Executor
//!
//! Validates Macie2 session and classification job configurations using both
//! scalar field checks and record check validation on the merged response.
...
//  pub use aws_macie2_account::AwsMacie2AccountExecutor;
//
//////////////////////////////////////////////////////
...
/// Executor for aws_macie2_account validation
pub struct AwsMacie2AccountExecutor {
    contract: CtnContract,
}

impl AwsMacie2AccountExecutor {
...
impl CtnExecutor for AwsMacie2AccountExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} Macie2 accounts, found {}",
        ...
        for (object_id, data) in &collected_data {
            ...
            for state in &criterion.states {
                ...
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg = "Macie2 not enabled, cannot validate record checks".to_string();
                        ...
                        failure_messages.push(format!("Macie2 '{}': {}", object_id, msg));
                        continue;
                    }
                    ...
                    for result in &validation_results {
                        ...
                        if !result.passed {
                            failure_messages
                                .push(format!("Macie2 '{}': {}", object_id, result.message));
                        }
                    }
                }
                ...
                for field in &state.fields {
                    ...
                    let data_field_name = self
                        ...
                        .get(&field.name)
                        .cloned()
                        // Dynamic fields (tag_key:*) fall through to direct name lookup
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Macie2 validation passed: {} of {} accounts compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "Macie2 validation failed:\n  - {}",
