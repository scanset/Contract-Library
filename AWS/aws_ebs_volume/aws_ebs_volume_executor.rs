...
//  pub use aws_ebs_volume::AwsEbsVolumeExecutor;
//
//////////////////////////////////////////////////////
...
pub struct AwsEbsVolumeExecutor {
    contract: CtnContract,
}

impl AwsEbsVolumeExecutor {
...
impl CtnExecutor for AwsEbsVolumeExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} backup volumes, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        let (state_results, failure_messages) =
            self.run_checks(criterion, &collected_data, "Volume");
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Backup vault validation passed: {} of {} volumes compliant",
                objects_passing,
                state_results.len()
            )
        } else {
