//! AWS Application Load Balancer (ALB) Executor
//!
//! Validates ALB configurations using both scalar field checks
...
//! ```rust
//! pub mod aws_alb;
//! pub use aws_alb::AwsAlbExecutor;
//! ```
...
/// Executor for aws_alb validation
pub struct AwsAlbExecutor {
...
impl CtnExecutor for AwsAlbExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} ALBs, found {}",
        ...
        for (object_id, data) in &collected_data {
            ...
            for state in &criterion.states {
                ...
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg = "ALB not found, cannot validate record checks".to_string();
                        ...
                        failure_messages.push(format!("ALB '{}': {}", object_id, msg));
                        continue;
                    }
                    ...
                    for result in &validation_results {
                        ...
                        if !result.passed {
                            failure_messages
                                .push(format!("ALB '{}': {}", object_id, result.message));
                        }
                    }
                }
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "ALB validation passed: {} of {} ALBs compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "ALB validation failed:\n  - {}",
