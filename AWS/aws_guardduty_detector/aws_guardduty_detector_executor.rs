//! AWS GuardDuty Detector Executor
//!
//! Validates GuardDuty detector configurations using both scalar field checks
//! and record check validation on the merged detector + destination response.
...
//  pub use aws_guardduty_detector::AwsGuardDutyDetectorExecutor;
//
//////////////////////////////////////////////////////
...
/// Executor for aws_guardduty_detector validation
pub struct AwsGuardDutyDetectorExecutor {
    contract: CtnContract,
}

impl AwsGuardDutyDetectorExecutor {
...
impl CtnExecutor for AwsGuardDutyDetectorExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} GuardDuty detectors, found {}",
        ...
        for (object_id, data) in &collected_data {
            ...
            for state in &criterion.states {
                ...
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg = "GuardDuty detector not found, cannot validate record checks"
                            .to_string();
                        ...
                        failure_messages.push(format!("Detector '{}': {}", object_id, msg));
                        continue;
                    }
                    ...
                    for result in &validation_results {
                        ...
                        if !result.passed {
                            failure_messages
                                .push(format!("Detector '{}': {}", object_id, result.message));
                        }
                    }
                }
                ...
                for field in &state.fields {
                    ...
                    let data_field_name = self
                        ...
                        .get(&field.name)
                        .cloned()
                        // Dynamic fields (tag_key:*, feature_*) fall through to direct name lookup
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "GuardDuty detector validation passed: {} of {} detectors compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "GuardDuty detector validation failed:\n  - {}",
