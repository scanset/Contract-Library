...
//  pub use aws_vpc_endpoint::AwsVpcEndpointExecutor;
//
//////////////////////////////////////////////////////
...
pub struct AwsVpcEndpointExecutor {
    contract: CtnContract,
}

impl AwsVpcEndpointExecutor {
...
impl CtnExecutor for AwsVpcEndpointExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} backup endpoints, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        let (state_results, failure_messages) =
            self.run_checks(criterion, &collected_data, "Endpoint");
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Backup vault validation passed: {} of {} endpoints compliant",
                objects_passing,
                state_results.len()
            )
        } else {
