...
//  pub use aws_backup_plan::AwsBackupPlanExecutor;
//
//////////////////////////////////////////////////////
...
pub struct AwsBackupPlanExecutor {
    contract: CtnContract,
}

impl AwsBackupPlanExecutor {
...
impl CtnExecutor for AwsBackupPlanExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} backup plans, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        let (state_results, failure_messages) = self.run_checks(criterion, &collected_data, "Plan");
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Backup vault validation passed: {} of {} plans compliant",
                objects_passing,
                state_results.len()
            )
        } else {
