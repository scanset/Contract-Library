//! AWS SSM Maintenance Window Executor
...
//  pub use aws_ssm_maintenance_window::AwsSsmMaintenanceWindowExecutor;
//
//////////////////////////////////////////////////////
...
pub struct AwsSsmMaintenanceWindowExecutor {
    contract: CtnContract,
}

impl AwsSsmMaintenanceWindowExecutor {
...
impl CtnExecutor for AwsSsmMaintenanceWindowExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} backup windows, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        let (state_results, failure_messages) =
            self.run_checks(criterion, &collected_data, "Window");
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Backup vault validation passed: {} of {} windows compliant",
                objects_passing,
                state_results.len()
            )
        } else {
