...
//  pub use aws_iam_user::AwsIamUserExecutor;
//
//////////////////////////////////////////////////////
...
pub struct AwsIamUserExecutor {
    contract: CtnContract,
}

impl AwsIamUserExecutor {
...
impl CtnExecutor for AwsIamUserExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} backup users, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        let (state_results, failure_messages) = self.run_checks(criterion, &collected_data, "User");
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Backup vault validation passed: {} of {} users compliant",
                objects_passing,
                state_results.len()
            )
        } else {
