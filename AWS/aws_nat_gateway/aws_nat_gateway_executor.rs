//! AWS NAT Gateway Executor
//!
//! Validates NAT Gateway configurations using both scalar field checks
...
//  pub use aws_nat_gateway::AwsNatGatewayExecutor;
//
//////////////////////////////////////////////////////
...
/// Executor for aws_nat_gateway validation
pub struct AwsNatGatewayExecutor {
    contract: CtnContract,
}

impl AwsNatGatewayExecutor {
...
impl CtnExecutor for AwsNatGatewayExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} NAT gateways, found {}",
        ...
        for (object_id, data) in &collected_data {
            ...
            for state in &criterion.states {
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg =
                            "NAT gateway not found, cannot validate record checks".to_string();
                        ...
                        failure_messages.push(format!("NAT '{}': {}", object_id, msg));
                        continue;
                    }
                    ...
                    for result in &validation_results {
                        ...
                        if !result.passed {
                            failure_messages
                                .push(format!("NAT '{}': {}", object_id, result.message));
                        }
                    }
                }
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "NAT gateway validation passed: {} of {} gateways compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "NAT gateway validation failed:\n  - {}",
