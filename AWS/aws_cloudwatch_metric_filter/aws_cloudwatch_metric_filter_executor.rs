//! AWS CloudWatch Metric Filter Executor
...
//  pub use aws_cloudwatch_metric_filter::AwsCloudwatchMetricFilterExecutor;
//
//////////////////////////////////////////////////////
...
pub struct AwsCloudwatchMetricFilterExecutor {
    contract: CtnContract,
}

impl AwsCloudwatchMetricFilterExecutor {
...
impl CtnExecutor for AwsCloudwatchMetricFilterExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} backup vaults, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        let (state_results, failure_messages) =
            self.run_checks(criterion, &collected_data, "Vault");
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Backup vault validation passed: {} of {} vaults compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "Backup vault validation failed:\n  - {}",
