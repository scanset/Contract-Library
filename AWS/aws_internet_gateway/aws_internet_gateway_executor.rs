//! AWS Internet Gateway Executor
//!
//! Validates Internet Gateway configurations using both scalar field checks
...
//  pub use aws_internet_gateway::AwsInternetGatewayExecutor;
//
//////////////////////////////////////////////////////
...
/// Executor for aws_internet_gateway validation
pub struct AwsInternetGatewayExecutor {
    contract: CtnContract,
}

impl AwsInternetGatewayExecutor {
...
impl CtnExecutor for AwsInternetGatewayExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} internet gateways, found {}",
        ...
        for (object_id, data) in &collected_data {
            ...
            for state in &criterion.states {
                ...
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg =
                            "Internet gateway not found, cannot validate record checks".to_string();
                        ...
                        failure_messages.push(format!("IGW '{}': {}", object_id, msg));
                        continue;
                    }
                    ...
                    for result in &validation_results {
                        ...
                        if !result.passed {
                            failure_messages
                                .push(format!("IGW '{}': {}", object_id, result.message));
                        }
                    }
                }
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Internet gateway validation passed: {} of {} gateways compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "Internet gateway validation failed:\n  - {}",
