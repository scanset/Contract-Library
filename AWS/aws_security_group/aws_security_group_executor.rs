//! AWS Security Group Executor
//!
//! Validates security group configurations using both scalar field checks
//! and record check validation on the full API response.
//!
//! Follows the k8s_resource executor pattern:
//! - Scalar fields (found, group_id, group_name, etc.) via compare_values()
//! - Record checks (IpPermissions, IpPermissionsEgress) via validate_record_checks()
...
//  pub use aws_security_group::AwsSecurityGroupExecutor;
//
//////////////////////////////////////////////////////
...
/// Executor for aws_security_group validation
pub struct AwsSecurityGroupExecutor {
    contract: CtnContract,
}

impl AwsSecurityGroupExecutor {
    pub fn new(contract: CtnContract) -> Self {
        Self { contract }
    }

    /// Compare values for scalar fields (found, group_id, counts, booleans)
    fn compare_values(
        &self,
        ...
        match (expected, actual, operation) {
            // Boolean comparisons (found, has_ingress_from_anywhere, has_egress_to_anywhere)
            ...
            // String comparisons (group_id, group_name, vpc_id, description)
            ...
            (ResolvedValue::String(exp), ResolvedValue::String(act), Operation::EndsWith) => {
                act.ends_with(exp.as_str())
            }
            (ResolvedValue::String(exp), ResolvedValue::String(act), Operation::NotContains) => {
                !act.contains(exp.as_str())
            }

            // Integer comparisons (ingress_rule_count, egress_rule_count)
...
impl CtnExecutor for AwsSecurityGroupExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} security groups, found {}",
        ...
        for (object_id, data) in &collected_data {
            ...
            for state in &criterion.states {
                ...
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg =
                            "Security group not found, cannot validate record checks".to_string();
                        ...
                        failure_messages.push(format!("SG '{}': {}", object_id, msg));
                        continue;
                    }
                    ...
                    for result in &validation_results {
                        ...
                        if !result.passed {
                            failure_messages
                                .push(format!("SG '{}': {}", object_id, result.message));
                        }
                    }
                }

                // Handle regular scalar field checks
                for field in &state.fields {
                    ...
                    // Skip record field — handled above via record checks
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Security group validation passed: {} of {} groups compliant",
                objects_passing,
                state_results.len()
            )
        } else {
