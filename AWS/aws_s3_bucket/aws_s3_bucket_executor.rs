//! AWS S3 Bucket Executor
//!
//! Validates S3 bucket configurations using both scalar field checks
//! and record check validation on the merged six-call response.
...
//  pub use aws_s3_bucket::AwsS3BucketExecutor;
//
//////////////////////////////////////////////////////
...
/// Executor for aws_s3_bucket validation
pub struct AwsS3BucketExecutor {
    contract: CtnContract,
}

impl AwsS3BucketExecutor {
...
impl CtnExecutor for AwsS3BucketExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} S3 buckets, found {}",
        ...
        for (object_id, data) in &collected_data {
            ...
            for state in &criterion.states {
                ...
                if !state.record_checks.is_empty() {
                    if !resource_found {
                        let msg = "S3 bucket not found, cannot validate record checks".to_string();
                        ...
                        failure_messages.push(format!("Bucket '{}': {}", object_id, msg));
                        continue;
                    }
                    ...
                    for result in &validation_results {
                        ...
                        if !result.passed {
                            failure_messages
                                .push(format!("Bucket '{}': {}", object_id, result.message));
                        }
                    }
                }
                ...
                for field in &state.fields {
                    // record is handled above via record_checks
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "S3 bucket validation passed: {} of {} buckets compliant",
                objects_passing,
                state_results.len()
            )
        } else {
