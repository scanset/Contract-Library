//! AWS Secrets Manager Secret Executor
...
//  pub use aws_secretsmanager_secret::AwsSecretsmanagerSecretExecutor;
//
//////////////////////////////////////////////////////
...
pub struct AwsSecretsmanagerSecretExecutor {
    contract: CtnContract,
}

impl AwsSecretsmanagerSecretExecutor {
...
impl CtnExecutor for AwsSecretsmanagerSecretExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} backup secrets, found {}",
                    criterion.expected_object_count(),
                    collected_data.len()
                ),
            )
        ...
        let (state_results, failure_messages) =
            self.run_checks(criterion, &collected_data, "Secret");
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Backup vault validation passed: {} of {} secrets compliant",
                objects_passing,
                state_results.len()
            )
        } else {
