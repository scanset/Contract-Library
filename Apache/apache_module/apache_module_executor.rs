...
//  pub use apache_module::ApacheModuleExecutor;
//
//////////////////////////////////////////////////////
...
pub struct ApacheModuleExecutor {
    contract: CtnContract,
}

impl ApacheModuleExecutor {
...
impl CtnExecutor for ApacheModuleExecutor {
    fn execute_with_contract(
        ...
        if !existence_passed {
            return Ok(CtnExecutionResult::fail(
                ...
                format!(
                    "Existence check failed: expected {} modules, found {}",
                ...
                for field in &state.fields {
                    ...
                    let actual_value = match data.get_field(&data_field_name) {
                        ...
                        None => {
                            ...
                            failure_messages.push(format!("Module '{}': {}", object_id, msg));
                            continue;
                        }
                    };
        ...
        let message = if final_status == Outcome::Pass {
            format!(
                "Apache module validation passed: {} of {} checks compliant",
                objects_passing,
                state_results.len()
            )
        } else {
            format!(
                "Apache module validation failed:\n  - {}",
    ...
    fn validate_collected_data(
        ...
        for data in collected_data.values() {
            if !data.has_field("loaded") {
                return Err(CtnExecutionError::MissingDataField {
                    field: "loaded".to_string(),
                });
            }
        }
        Ok(())
